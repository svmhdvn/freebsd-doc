...
Possible arguments: (none), `base` (default), `heimdal`, `mit`, `heimdal-devel`, `mit-devel`, `flags`, `bootstrap`

Handle dependencies needed by consumers of the GSSAPI.
Only libraries that provide the Kerberos mechanism are available.
By default, or when set to `base`, the GSSAPI library from the base system is used.
Can also be set to `heimdal` to use package:security/heimdal[], or `mit` to use package:security/krb5[].
The `heimdal-devel` and `mit-devel` counterparts can be set to use package:security/heimdal-devel[] or package:security/krb5-devel[] respectively.

When the local Kerberos installation is not in `LOCALBASE`, set `HEIMDAL_HOME` (for `heimdal` or `heimdal-devel`) or `KRB5_HOME` (for `krb5` or `krb5-devel`) to the location of the Kerberos installation.

The default version is set by the usual `DEFAULT_VERSIONS` mechanism, unless an alternate provider is specified as an argument (for example, `USES=gssapi:heimdal-devel`).
...
The `flags` option can be set after the primary provider argument to automatically add `GSSAPICPPFLAGS`, `GSSAPILDFLAGS`, and `GSSAPILIBS` to `CFLAGS`, `LDFLAGS`, and `LDADD`, respectively.
For example, use `base,flags`.

The `bootstrap` option is a special prefix only for use before provider arguments that are not `base`.
...
[[uses-gssapi-ex2]]
.Overriding the DEFAULT_VERSIONS mechanism with only specific providers supported
[example]
====
[.programlisting]
....
# Only support heimdal-based providers from the Ports collection
